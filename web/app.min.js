...
a,!1):this.fireEvent("selectReport",a,!1))},showArea:function(a){for(var b=this.getView().getVectorSource(),c=[],e=0;e<a.coordinates.length;e++){var d=a.coordinates[e];c.push([d.longitude,d.latitude])}a=new ol.geom.Polygon([c]);a=new ol.Feature(a);b.clear();b.addFeature(a)},drawArea:function(a){var b=this.getView().getVectorSource();b.clear();var c=this.getView().getMap(),e;if("None"!==a){var d;"Circle"===a?d=ol.interaction.Draw.createRegularPolygon(32):"Square"===a&&(a="Circle",d=ol.interaction.Draw.createRegularPolygon(4));
e=new ol.interaction.Draw({source:b,type:a,geometryFunction:d,maxPoints:void 0});c.addInteraction(e);e.on("drawend",function(b){b=b.feature.getGeometry().getCoordinates()[0];for(var d=[],g=0;g<b.length;g++)d.push({longitude:b[g][0],latitude:b[g][1]});Ext.Msg.prompt("Name","Area name:",function(b,f){if("ok"==b&&f){var g={type:a,name:f,coordinates:d};store=Ext.getStore("Polygons");store.add(g);Ext.toast("Polygon saved");store.sync()}c.removeInteraction(e)})})}}});
Ext.define("Traccar.view.Map",{extend:Ext.form.Panel,xtype:"mapView",header:!1,controller:"map",title:Strings.mapTitle,layout:"fit",getMap:function(){return this.map},getMapView:function(){return this.mapView},getLatestSource:function(){return this.latestSource},getRouteSource:function(){return this.routeSource},getReportSource:function(){return this.reportSource},getVectorSource:function(){return this.vectorSource},listeners:{afterrender:function(){var a,b,c,e,d,f,h,g,m;d=function(a,b){return new ol.layer.Tile({source:new ol.source.XYZ({url:a,
attributions:[new ol.Attribution({html:""})]}),projection:b,visible:!1})};f=function(a){return new ol.layer.Tile({source:new ol.source.BingMaps({key:"Atabw4WpqrmFXz8yRA9yxMzk2u--7Znu5POGdRsivAkyFw-6QAeOtwgMU8Upcb-W",imagerySet:a}),visible:!1})};var k=new ol.layer.Tile({source:new ol.source.OSM({}),visible:!1});e=f("Road");f=f("Aerial");var l=d("http://mt0.google.com/vt/lyrs\x3dy\x26hl\x3den\x26x\x3d{x}\x26y\x3d{y}\x26z\x3d{z}\x26s\x3dGa"),n=d("http://mt0.google.com/vt/lyrs\x3ds\x26hl\x3den\x26x\x3d{x}\x26y\x3d{y}\x26z\x3d{z}\x26s\x3dGa");
h=d("http://mt0.google.com/vt/lyrs\x3dm\x26hl\x3den\x26x\x3d{x}\x26y\x3d{y}\x26z\x3d{z}\x26s\x3dGa");g=d("http://services.arcgisonline.com/ArcGIS/rest/services/World_Topo_Map/MapServer/tile/{z}/{y}/{x}.png");d=d("http://services.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}.png");var j=[{name:"OpenStreetMaps",layer:k},{name:"Bing Road",layer:e},{name:"Bing Areal",layer:f},{name:"Google Roads",layer:h},{name:"Google Satelite",layer:n},{name:"Google Hybrid",layer:l},
{name:"ArcGis Topo",layer:g},{name:"ArcGis Imagery",layer:d}],k=function(a){var b=document.createElement("button");b.className="ol-save-button fa-floppy-o";b.title="Save location";b.addEventListener("click",function(){for(var b,c=0;c<j.length;++c)j[c].layer.getVisible()&&(b=j[c].name);Ext.Ajax.request({scope:this,url:"/api/user/location/update",method:"POST",params:{map:b,zoom:a.getView().getZoom(),longitude:ol.proj.toLonLat(a.getView().getCenter())[0],latitude:ol.proj.toLonLat(a.getView().getCenter())[1]},
callback:function(){Ext.toast("Location saved")}})},!1);var c=document.createElement("div");c.className="ol-unselectable ol-control ol-save-div";c.appendChild(b);ol.control.Control.call(this,{element:c})};ol.inherits(k,ol.control.Control);l=function(){var a=document.createElement("select");a.className="ol-layers-select";for(var b=0,d=0;d<j.length;++d){var e=document.createElement("option");e.text=j[d].name;e.value=j[d].name;j[d].name==c&&(b=d,j[d].layer.setVisible(!0));a.add(e)}a.selectedIndex=b;
a.addEventListener("change",function(){for(var b=0;b<j.length;++b)j[b].layer.setVisible(a.value===j[b].name)},!1);b=document.createElement("div");b.className="ol-unselectable ol-control ol-layers-div";b.appendChild(a);ol.control.Control.call(this,{element:b})};ol.inherits(l,ol.control.Control);a=Traccar.app.getUser();b=Traccar.app.getServer();c=a.get("map")||b.get("map");this.latestSource=new ol.source.Vector({});e=new ol.layer.Vector({source:this.latestSource});this.routeSource=new ol.source.Vector({});
...
url:"/api/session",callback:this.onSessionReturn})):Traccar.app.showError(c)},onSessionReturn:function(a,b,c){b?(Traccar.app.setUser(Ext.decode(c.responseText)),this.loadApp()):(this.login=Ext.create("widget.login",{listeners:{scope:this,login:this.onLogin}}),this.login.show())},onLogin:function(){this.login.close();this.loadApp()},loadApp:function(){Ext.getStore("Devices").load();Ext.getStore("Polygons").load();Ext.get("attribution").remove();this.isPhone?Ext.create("widget.mainMobile"):Ext.create("widget.main");
this.startWebSocket()},startWebSocket:function(){var a;a="https:"===location.protocol?"wss:":"ws:";a+="//"+location.host;a+=location.pathname+"api/ws/positions/";websocket=new WebSocket(a);websocket.onopen=function(){Ext.toast("Connection to server established")};websocket.onclose=function(){Ext.toast("Connection to server closed")};websocket.onmessage=function(a){var c,e,d,f,h;c=Ext.getStore("Devices");e=Ext.getStore("LatestPositions");a=Ext.decode(a.data).data;d=a.devices;f=a.positions;for(a=0;a<
d.length;a++)(h=c.findRecord("id",d[a].id,0,!1,!1,!0))&&h.set({status:d[a].status,lastUpdate:d[a].lastUpdate},{dirty:!1});for(a=0;a<f.length;a++)(c=e.findRecord("deviceId",f[a].deviceId,0,!1,!1,!0))?c.set(f[a]):e.add(Ext.create("Traccar.model.Position",f[a]))};websocket.onerror=function(a){Ext.toast("Server connection error: "+a.data)}}});
Ext.define("Traccar.Application",{extend:Ext.app.Application,name:"Traccar",models:"Server User Device Position Attribute Command Polygon".split(" "),stores:"Devices AllDevices Positions LatestPositions Users Attributes MapTypes DistanceUnits SpeedUnits CommandTypes TimeUnits Languages Polygons".split(" "),controllers:["Root"],setUser:function(a){this.user=Ext.create("Ext.data.reader.Json",{model:"Traccar.model.User"}).readRecords(a).getRecords()[0]},getUser:function(){return this.user},setServer:function(a){this.server=
Ext.create("Ext.data.reader.Json",{model:"Traccar.model.Server"}).readRecords(a).getRecords()[0]},getServer:function(){return this.server},getPreference:function(a,b){return this.getUser().get(a)||this.getServer().get(a)||b},showError:function(a){Ext.isString(a)?Ext.Msg.alert(Strings.errorTitle,a):a.responseText?(a=Ext.decode(a.responseText),a.details?Ext.Msg.alert(Strings.errorTitle,a.details):Ext.Msg.alert(Strings.errorTitle,a.message)):a.statusText?Ext.Msg.alert(Strings.errorTitle,a.statusText):
Ext.Msg.alert(Strings.errorTitle,Strings.errorConnection)}});Ext.application({name:"Traccar",extend:Traccar.Application});