...
Ext.define("Traccar.view.RegisterController",{extend:Ext.app.ViewController,alias:"controller.register",init:function(){this.lookupReference("languageField").setValue(Locale.language)},onCreateClick:function(){var a=this.lookupReference("form"),b=a.getValues();b.personal=!!b.personal&&"on"==b.personal;b.admin=!0;a.isValid()&&Ext.Ajax.request({scope:this,method:"POST",url:"./api/users/register",jsonData:b,callback:this.onCreateReturn})},onCreateReturn:function(a,b,c){b?(this.closeView(),Ext.toast(Strings.loginCreated)):
showError(c)},onPersonalClick:function(){var a=this.lookupReference("company");a.setDisabled(!1);a.focus()},onSelectLanguage:function(a){var b,c;a=a.getValue();b=window.location.href;0<=b.indexOf("locale\x3d")?(c=b.substring(0,b.indexOf("locale")),b=b.substring(b.indexOf("locale")),b=b.substring(b.indexOf("\x3d")+1),b=0<=b.indexOf("\x26")?b.substring(b.indexOf("\x26")):"",b=c+"locale\x3d"+a+b):b=0>b.indexOf("?")?b+("?locale\x3d"+a):b+("\x26locale\x3d"+a);window.location.href=b},showError:function(a){Ext.isString(a)?
Ext.Msg.alert(Strings.errorTitle,a):a.responseText?(a=Ext.decode(a.responseText),a.details?Ext.Msg.alert(Strings.errorTitle,a.details):Ext.Msg.alert(Strings.errorTitle,a.message)):a.statusText?Ext.Msg.alert(Strings.errorTitle,a.statusText):Ext.Msg.alert(Strings.errorTitle,Strings.errorConnection)}});
...
tooltipType:"title",handler:"onUsersClick",reference:"settingsUsersButton"},{glyph:"xf013@FontAwesome",tooltip:"Account",tooltipType:"title",handler:"onAccoountClick"},{glyph:"xf08b@FontAwesome",tooltip:"Sign out",tooltipType:"title",handler:"onLogoutClick"}]},listeners:{selectionchange:"onSelectionChange"},columns:[{text:Strings.deviceName,dataIndex:"name",flex:1},{text:Strings.deviceLastUpdate,dataIndex:"lastUpdate",flex:1,renderer:function(a,b,c){switch(c.get("status")){case "online":b.tdCls="status-color-online";
break;case "offline":b.tdCls="status-color-offline";break;default:b.tdCls="status-color-unknown"}return Ext.Date.format(a,Traccar.Style.dateTimeFormat)}}]});
...
flex:1}]});
Ext.define("Traccar.view.StateController",{extend:Ext.app.ViewController,alias:"controller.state",config:{listen:{controller:{"*":{selectDevice:"selectDevice",selectReport:"selectReport"}},store:{"#LatestPositions":{add:"updateLatest",update:"updateLatest"},"#Positions":{clear:"clearReport"}}}},keys:function(){var a,b,c;c={};b="fixTime latitude longitude valid altitude speed course address protocol".split(" ");for(a=0;a<b.length;a++)c[b[a]]={priority:a,name:Strings["position"+b[a].replace(/^\w/g,function(a){return a.toUpperCase()})]};
return c}(),updateLatest:function(a,b){var c;Ext.isArray(b)||(b=[b]);for(c=0;c<b.length;c++)this.deviceId===b[c].get("deviceId")&&this.updatePosition(b[c])},formatValue:function(a){return"number"===typeof id?Number(a.toFixed(2)):a},updatePosition:function(a){var b,c;b=Ext.getStore("Attributes");b.removeAll();for(c in a.data)a.data.hasOwnProperty(c)&&void 0!==this.keys[c]&&b.add(Ext.create("Traccar.model.Attribute",{priority:this.keys[c].priority,name:this.keys[c].name,value:Traccar.AttributeFormatter.getFormatter(c)(a.get(c))}));
a=a.get("attributes");if(a instanceof Object)for(c in a)a.hasOwnProperty(c)&&b.add(Ext.create("Traccar.model.Attribute",{priority:1024,name:c.replace(/^./,function(a){return a.toUpperCase()}),value:Traccar.AttributeFormatter.getFormatter(c)(a[c])}))},selectDevice:function(a){this.deviceId=a.get("id");(a=Ext.getStore("LatestPositions").findRecord("deviceId",this.deviceId,0,!1,!1,!0))?this.updatePosition(a):Ext.getStore("Attributes").removeAll()},selectReport:function(a){this.deviceId=null;this.updatePosition(a)},
clearReport:function(){Ext.getStore("Attributes").removeAll()}});Ext.define("Traccar.view.State",{extend:Ext.grid.Panel,xtype:"stateView",controller:"state",store:"Attributes",title:Strings.stateTitle,columns:[{text:Strings.stateName,dataIndex:"name",flex:1},{text:Strings.stateValue,dataIndex:"value",flex:1}]});
...
projection:b,visible:!1})};f=function(a){return new ol.layer.Tile({source:new ol.source.BingMaps({key:"Atabw4WpqrmFXz8yRA9yxMzk2u--7Znu5POGdRsivAkyFw-6QAeOtwgMU8Upcb-W",imagerySet:a}),visible:!1})};var l=new ol.layer.Tile({source:new ol.source.OSM({}),visible:!1});e=f("Road");f=f("Aerial");var m=d("http://mt0.google.com/vt/lyrs\x3dy\x26hl\x3den\x26x\x3d{x}\x26y\x3d{y}\x26z\x3d{z}\x26s\x3dGa"),n=d("http://mt0.google.com/vt/lyrs\x3ds\x26hl\x3den\x26x\x3d{x}\x26y\x3d{y}\x26z\x3d{z}\x26s\x3dGa");h=d("http://mt0.google.com/vt/lyrs\x3dm\x26hl\x3den\x26x\x3d{x}\x26y\x3d{y}\x26z\x3d{z}\x26s\x3dGa");
g=d("http://services.arcgisonline.com/ArcGIS/rest/services/World_Topo_Map/MapServer/tile/{z}/{y}/{x}.png");d=d("http://services.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}.png");var j=[{name:"OpenStreetMaps",layer:l},{name:"Bing Road",layer:e},{name:"Bing Areal",layer:f},{name:"Google Roads",layer:h},{name:"Google Satelite",layer:n},{name:"Google Hybrid",layer:m},{name:"ArcGis Topo",layer:g},{name:"ArcGis Imagery",layer:d}],l=function(a){var b=document.createElement("button");
b.className="ol-save-button fa-floppy-o";b.title="Save location";b.addEventListener("click",function(){for(var b,c=0;c<j.length;++c)j[c].layer.getVisible()&&(b=j[c].name);Ext.Ajax.request({scope:this,url:"./api/users/location/update",method:"POST",params:{map:b,zoom:a.getView().getZoom(),longitude:ol.proj.toLonLat(a.getView().getCenter())[0],latitude:ol.proj.toLonLat(a.getView().getCenter())[1]},callback:function(){Ext.toast("Location saved")}})},!1);var c=document.createElement("div");c.className=
"ol-unselectable ol-control ol-save-div";c.appendChild(b);ol.control.Control.call(this,{element:c})};ol.inherits(l,ol.control.Control);m=function(){var a=document.createElement("select");a.className="ol-layers-select";for(var b=0,d=0;d<j.length;++d){var e=document.createElement("option");e.text=j[d].name;e.value=j[d].name;j[d].name==c&&(b=d,j[d].layer.setVisible(!0));a.add(e)}a.selectedIndex=b;a.addEventListener("change",function(){for(var b=0;b<j.length;++b)j[b].layer.setVisible(a.value===j[b].name)},
!1);b=document.createElement("div");b.className="ol-unselectable ol-control ol-layers-div";b.appendChild(a);ol.control.Control.call(this,{element:b})};ol.inherits(m,ol.control.Control);a=Traccar.app.getUser();b=Traccar.app.getServer();c=a.get("map")||b.get("map");this.latestSource=new ol.source.Vector({});e=new ol.layer.Vector({source:this.latestSource});this.routeSource=new ol.source.Vector({});d=new ol.layer.Vector({source:this.routeSource});this.reportSource=new ol.source.Vector({});f=new ol.layer.Vector({source:this.reportSource});
this.vectorSource=new ol.source.Vector({wrapX:!1});n=new ol.layer.Vector({source:this.vectorSource,style:new ol.style.Style({fill:new ol.style.Fill({color:"rgba(255, 255, 255, 0.2)"}),stroke:new ol.style.Stroke({color:"#ffcc33",width:2}),image:new ol.style.Circle({radius:7,fill:new ol.style.Fill({color:"#ffcc33"})})})});h=a.get("latitude")||b.get("latitude")||Traccar.Style.mapDefaultLat;g=a.get("longitude")||b.get("longitude")||Traccar.Style.mapDefaultLon;a=a.get("zoom")||b.get("zoom")||Traccar.Style.mapDefaultZoom;
this.mapView=new ol.View({center:ol.proj.fromLonLat([g,h]),zoom:a,maxZoom:Traccar.Style.mapMaxZoom});h=[];for(g=0;g<j.length;++g)h.push(j[g].layer);h.push(d);h.push(f);h.push(e);h.push(n);this.map=new ol.Map({target:this.body.dom.id,layers:h,view:this.mapView,interactions:ol.interaction.defaults().extend([new ol.interaction.DragRotateAndZoom])});m=new m(this.map);this.map.addControl(m);e=new ol.control.ScaleLine;this.map.addControl(e);e=new ol.control.ZoomSlider;this.map.addControl(e);e=new ol.control.MousePosition({projection:ol.proj.get("EPSG:4326"),
coordinateFormat:function(a){return a[0].toFixed(6)+" "+a[1].toFixed(6)}});this.map.addControl(e);e=new ol.control.FullScreen;this.map.addControl(e);l=new l(this.map);this.map.addControl(l);k=this.map.getTarget();"string"===typeof k&&(k=Ext.get(k).dom);this.map.on("pointermove",function(a){k.style.cursor=this.forEachFeatureAtPixel(a.pixel,function(){return!0})?"pointer":""});this.map.on("click",function(a){this.map.forEachFeatureAtPixel(a.pixel,function(a){this.fireEvent("selectFeature",a)},this)},
this)},resize:function(){this.map.updateSize()}}});
...
this.loadApp()):location.href="login.html"):Traccar.app.showError(c)},loadApp:function(){Ext.getStore("Devices").load();Ext.getStore("Alerts").load();Ext.getStore("Polygons").load();this.isPhone?Ext.create("widget.mainMobile"):Ext.create("widget.main");this.startWebSocket()},startWebSocket:function(){var a;a="https:"===location.protocol?"wss:":"ws:";a+="//"+location.host;a+=location.pathname.replace("/debug.html","/")+"ws/positions";websocket=new WebSocket(a);websocket.onopen=function(){Ext.toast("Connection to server established")};
websocket.onclose=function(){Ext.toast("Connection to server closed")};websocket.onmessage=function(a){var c,e,d,f,h;message=Ext.decode(a.data);a=message.body;if("position"==message.type){d=a.devices;f=a.positions;c=Ext.getStore("Devices");e=Ext.getStore("LatestPositions");for(a=0;a<d.length;a++)(h=c.findRecord("id",d[a].id,0,!1,!1,!0))&&h.set({status:d[a].status,lastUpdate:d[a].lastUpdate},{dirty:!1});for(a=0;a<f.length;a++)(d=e.findRecord("deviceId",f[a].deviceId,0,!1,!1,!0))?d.set(f[a]):e.add(Ext.create("Traccar.model.Position",
f[a]))}else"alert"==message.type&&(alertsStore=Ext.getStore("Alerts"),alertsStore.add(Ext.create("Traccar.model.Alert",a.message)))};websocket.onerror=function(a){Ext.toast("Server connection error: "+a.data)}}});
